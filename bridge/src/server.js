...
export function createBridgeServer({ config, acp, serviceOptions, machineRegistry }) {
  ...
  return http.createServer(async (request, response) => {
    ...
    if (!matchesCredentials(request, config)) { response.writeHead(401, { "WWW-Authenticate": 'Basic realm="Harness Remote Bridge"' }); response.end(); return }
    ...
    try {
      ...
      if (request.method === "GET" && url.pathname === "/session/status") { const statuses = Object.fromEntries((await service.listSessions(directory)).map((session) => [session.id, service.status(session.id)])); writeJSON(response, 200, statuses); return }
      ...
      if (request.method === "POST" && url.pathname === "/session") { const body = await readBody(request); const selected = await allowedDirectory(directory ?? config.roots[0] ?? process.cwd(), config); const created = await service.createSession({ directory: selected, title: body.title, model: modelWireName(body.model) }); writeJSON(response, 200, created); return }
      const sessionMatch = /^\/session\/([^/]+)(?:\/(message|prompt_async|abort|todo|diff|action|command)(?:\/([^/]+))?)?$/.exec(url.pathname)
      if (sessionMatch) { const [, sessionID, operation, actionID] = sessionMatch; if (request.method === "PATCH" && !operation) { const body = await readBody(request); writeJSON(response, 200, await service.renameSession(sessionID, typeof body.title === "string" ? body.title : "")); return }; if (request.method === "DELETE" && !operation) { await service.deleteSession(sessionID); writeJSON(response, 200, true); return }; if (request.method === "GET" && operation === "message") { writeJSON(response, 200, await service.messages(sessionID, url.searchParams.get("refresh") === "1")); return }; if (request.method === "GET" && operation === "todo") { writeJSON(response, 200, await service.todos(sessionID)); return }; if (request.method === "GET" && operation === "diff") { writeJSON(response, 200, []); return }; if (request.method === "GET" && operation === "action" && !actionID) { writeJSON(response, 200, await service.actions(sessionID)); return }; if (request.method === "POST" && operation === "action" && actionID) { writeJSON(response, 200, await service.invokeAction(sessionID, actionID)); return }; if (request.method === "POST" && operation === "prompt_async") { const body = await readBody(request); const text = body.parts?.find((part) => part.type === "text")?.text ?? ""; const attachments = parseAttachments(body.parts); if (!text && !attachments.length) throw new Error("A text prompt is required"); await service.prompt(sessionID, text, modelWireName(body.model), attachments); writeJSON(response, 200, true); return }; if (request.method === "POST" && operation === "command") { const body = await readBody(request); if (typeof body.command !== "string" || !body.command) throw new Error("A command name is required"); const argumentsText = typeof body.arguments === "string" ? body.arguments.trim() : ""; const text = argumentsText ? `/${body.command} ${argumentsText}` : `/${body.command}`; await service.prompt(sessionID, text, modelWireName(body.model)); writeJSON(response, 200, true); return }; if (request.method === "POST" && operation === "abort") { service.abort(sessionID); writeJSON(response, 200, true); return } }
      ...
      if (request.method === "GET" && url.pathname === "/config/providers") { const sessionID = url.searchParams.get("sessionID"); if (!sessionID) { writeJSON(response, 200, { providers: [], default: {} }); return }; writeJSON(response, 200, providersResponse(await service.models(sessionID), backend)); return }
