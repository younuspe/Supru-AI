...
export const SUPRU_PROFILES = {
  omp: {
    id: "omp",
    label: "Oh My Pi",
    command: bundledOmpCommand(),
    args: ["acp"],
...
export function supruProfile(id) {
  const profile = SUPRU_PROFILES[id]
  if (!profile) throw new Error(`Unsupported backend: ${id}`)
  return profile
}

export function resolveAcpLaunch(profile, { find = findExecutable } = {}) {
  if (!profile.adapterCommand) return { command: profile.command, args: [...profile.args], source: "supru" }
