...
function parseArgumentList(value, fallback) {
  ...
  if (!Array.isArray(parsed) || parsed.some((argument) => typeof argument !== "string")) throw new Error("ACP arguments must be a JSON array of strings")
  return parsed
}

function parseBackend(value) { return harnessProfile(value).id }

function environmentValue(environment, name) {
  return environment[`SUPRU_AI_${name}`] ?? environment[`HARNESS_REMOTE_${name}`] ?? environment[`OMP_BRIDGE_${name}`]
}
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    switch (option) {
      case "--backend":
        ...
        { const selected = resolveAcpLaunch(harnessProfile(config.backend)); if (!acpCommandOverridden) config.acpCommand = selected.command; if (!acpArgsOverridden) config.acpArgs = [...selected.args] }
        index += 1; break
