...
assert.match(desktopBridge, /window\.supruDesktop/, 'Electron detection must use Supru AI preload marker')
assert.match(main, /!window\.supruDesktop\?\.platform\.isDesktop/, 'Electron must skip service-worker registration')
assert.doesNotMatch(desktopBridge, /window\.harnessDesktop/, 'Legacy Harness preload marker must be removed')
assert.doesNotMatch(main, /window\.harnessDesktop/, 'Legacy Harness preload marker must be removed')
assert.doesNotMatch(desktopBridge, /harness/i, 'Desktop bridge must not contain legacy Harness branding')
assert.doesNotMatch(main, /harness/i, 'Desktop entrypoint must not contain legacy Harness branding')
