...
function unauthorizedDetail(config: ServerConfig): string {
  return hasCredentials(config) ? "HTTP 401: the server rejected these credentials." : "HTTP 401: this server requires a username and password, and none were sent."
}
...
function responseDetail(body: unknown): string | null {
  if (!body) return null
  if (typeof body === "string") { try { return responseDetail(JSON.parse(body)) ?? body } catch { return body } }
...
type ConfigProvidersResponse = { providers: Array<{ id: string; name: string; models: Record<string, { id?: string; name?: string; description?: string; status?: string; capabilities?: { attachment?: boolean; toolcall?: boolean; tools?: boolean }; limit?: { context?: number; output?: number }; variants?: Record<string, unknown> }> }>; default?: Record<string, string> }
type AgentResponse = Array<{ id?: string; name?: string; description?: string; mode: "primary" | "subagent" | "all"; hidden?: boolean }>
...
async function requestWithHeaders<T>(config: ServerConfig, path: string, options: RequestOptions = {}): Promise<ResponseWithHeaders<T>> {
  ...
  if (Capacitor.isNativePlatform()) {
    let response
    try { response = await CapacitorHttp.request({ url: target, method, headers, data: options.body, connectTimeout: 12_000, readTimeout: options.readTimeout ?? 30_000 }) }
    catch { throw new Error(`Cannot reach ${config.host}:${config.port}.`) }
    if (response.status >= 400) { if (response.status === 401) throw new Error(responseDetail(response.data) || unauthorizedDetail(config)); throw new Error(responseDetail(response.data) || `HTTP ${response.status}`) }
  ...
  try { response = await fetch(target, { method, headers, body: options.body === undefined ? undefined : JSON.stringify(options.body) }) }
  catch { const corsHint = hasCredentials(config) ? " In a browser, Basic Auth also needs the bridge started with --cors for this origin." : ""; throw new Error(`Cannot reach ${config.host}:${config.port}.${corsHint}`) }
  if (!response.ok) {
    let detail = response.status === 401 ? unauthorizedDetail(config) : `HTTP ${response.status}`
    try { detail = responseDetail(await response.text()) ?? detail } catch {}
...
async function request<T>(config: ServerConfig, path: string, options: RequestOptions = {}): Promise<T> { return (await requestWithHeaders<T>(config, path, options)).data }
function toAgentOption(agent: AgentResponse[number]): AgentOption { const id = agent.id || agent.name || ""; return { id, name: agent.name || id, description: agent.description, mode: agent.mode, hidden: agent.hidden } }
function toModelBody(model?: ModelSelection) { if (!model) return undefined; return { providerID: model.providerID, modelID: model.modelID } }
function toCreateSessionModel(model?: ModelSelection) { if (!model) return undefined; return { providerID: model.providerID, id: model.modelID, variant: model.variant || undefined } }
function modelWireName(model?: ModelSelection) { if (!model) return undefined; return `${model.providerID}/${model.modelID}` }
...
export const api = {
  ...
  loadPath(config: ServerConfig, directory?: string) { return request<PathInfo>(config, withDirectory("/path", directory)) },
  listFiles(config: ServerConfig, path: string, directory?: string) { return request<FileEntry[]>(config, withDirectory(`/file?path=${encodeURIComponent(path)}`, directory)) },
  ...
  deleteSession(config: ServerConfig, id: string, directory?: string) { return request<boolean>(config, withDirectory(`/session/${id}`, directory), { method: "DELETE" }) },
  loadMessages(config: ServerConfig, sessionID: string, directory?: string, refreshHistory = false) { const refresh = refreshHistory ? "&refresh=1" : ""; return request<MessageEnvelope[]>(config, withDirectory(`/session/${sessionID}/message?limit=100${refresh}`, directory)) },
  loadLatestMessage(config: ServerConfig, sessionID: string, directory?: string) { return request<MessageEnvelope[]>(config, withDirectory(`/session/${sessionID}/message?limit=1`, directory)) },
  loadTodo(config: ServerConfig, sessionID: string, directory?: string) { return request<TodoItem[]>(config, withDirectory(`/session/${sessionID}/todo`, directory)) },
  loadDiff(config: ServerConfig, sessionID: string, directory?: string) { return request<DiffFile[]>(config, withDirectory(`/session/${sessionID}/diff`, directory)) },
  loadMessageDiff(config: ServerConfig, sessionID: string, messageID: string, directory?: string) { return request<DiffFile[]>(config, withDirectory(`/session/${sessionID}/diff?messageID=${encodeURIComponent(messageID)}`, directory)) },
  ...
  unrevertSession(config: ServerConfig, sessionID: string, directory?: string) { return request<Session>(config, withDirectory(`/session/${sessionID}/unrevert`, directory), { method: "POST", body: {} }) },
  abort(config: ServerConfig, sessionID: string, directory?: string) { return request<boolean>(config, withDirectory(`/session/${sessionID}/abort`, directory), { method: "POST", body: {} }) },
  loadQuestions(config: ServerConfig, directory?: string) { return request<QuestionRequest[]>(config, withDirectory("/question", directory)) },
  replyQuestion(config: ServerConfig, requestID: string, answers: string[][], directory?: string) { return request<boolean>(config, withDirectory(`/question/${requestID}/reply`, directory), { method: "POST", body: { answers } }) },
  rejectQuestion(config: ServerConfig, requestID: string, directory?: string) { return request<boolean>(config, withDirectory(`/question/${requestID}/reject`, directory), { method: "POST", body: {} }) },
  loadPermissions(config: ServerConfig, directory?: string) { return request<PermissionRequest[]>(config, withDirectory("/permission", directory)) },
  replyPermission(config: ServerConfig, requestID: string, reply: "once" | "always" | "reject", directory?: string) { return request<boolean>(config, withDirectory(`/permission/${requestID}/reply`, directory), { method: "POST", body: { reply } }) }
}
