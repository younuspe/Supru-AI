...
function installIPC(): void {
  ...
  ipcMain.handle(IPC_CHANNELS.subscribeEvents, async (event, profileId: unknown, options: unknown) => { ensureTrustedSender(event); if (typeof profileId !== "string" || !options || typeof options !== "object") throw new Error("Event payload is invalid"); return eventTransport.subscribe(event.sender, profileId, options as DesktopEventSubscriptionOptions) })
